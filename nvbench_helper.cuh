...
template <class T, class... As> struct push_back<T, nvbench::type_list<As...>> {
...
using fundamental_types = nvbench::type_list<int8_t, int16_t, int32_t, int64_t,
#if NVBENCH_HELPER_HAS_I128
                                             int128_t,
#endif
                                             float, double>;

using all_types = nvbench::type_list<int8_t, int16_t, int32_t, int64_t,
#if NVBENCH_HELPER_HAS_I128
                                     int128_t,
#endif
...
template <class T> class value_wrapper_t {
  T m_val{};
...
template <typename T>
void gen_host(seed_t seed, cuda::std::span<T> data, bit_entropy entropy, T min,
              T max);

template <typename T>
void gen_device(seed_t seed, cuda::std::span<T> data, bit_entropy entropy,
                T min, T max);

template <typename T>
void gen_uniform_key_segments_host(seed_t seed, cuda::std::span<T> data,
                                   std::size_t min_segment_size,
                                   std::size_t max_segment_size);

template <typename T>
void gen_uniform_key_segments_device(seed_t seed, cuda::std::span<T> data,
                                     std::size_t min_segment_size,
                                     std::size_t max_segment_size);
...
std::size_t gen_uniform_segment_offsets_device(
    seed_t seed, cuda::std::span<T> segment_offsets,
    std::size_t min_segment_size, std::size_t max_segment_size);
...
struct generator_base_t {
  ...
  template <typename T> thrust::device_vector<T> generate(T min, T max) {
...
template <class T> struct vector_generator_t : generator_base_t {
...
template <> struct vector_generator_t<void> : generator_base_t {
  template <typename T> operator thrust::device_vector<T>() {
    return generator_base_t::generate(std::numeric_limits<T>::min(),
                                      std::numeric_limits<T>::max());
  }

  // This overload is needed because numeric limits is not specialized for
  // complex, making the min and max values for complex equal zero.
  operator thrust::device_vector<complex>() {
    const complex min =
        complex{std::numeric_limits<complex::value_type>::min(),
                std::numeric_limits<complex::value_type>::min()};
    const complex max =
        complex{std::numeric_limits<complex::value_type>::max(),
...
struct uniform_key_segments_generator_t {
  ...
  template <class KeyT> operator thrust::device_vector<KeyT>() {
    thrust::device_vector<KeyT> keys_vec(m_total_elements);
    cuda::std::span<KeyT> keys(thrust::raw_pointer_cast(keys_vec.data()),
                               keys_vec.size());
#if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
    gen_uniform_key_segments_device(m_seed, keys, m_min_segment_size,
                                    m_max_segment_size);
#else
    gen_uniform_key_segments_host(m_seed, keys, m_min_segment_size,
                                  m_max_segment_size);
#endif
    ++m_seed;
    return keys_vec;
...
struct uniform_segment_offsets_generator_t {
  ...
  template <class OffsetT> operator thrust::device_vector<OffsetT>() {
    thrust::device_vector<OffsetT> offsets_vec(m_total_elements + 2);
    cuda::std::span<OffsetT> offsets(
        thrust::raw_pointer_cast(offsets_vec.data()), offsets_vec.size());
    const std::size_t offsets_size =
#if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
        gen_uniform_segment_offsets_device(m_seed, offsets, m_min_segment_size,
                                           m_max_segment_size);
#else
        gen_uniform_segment_offsets_host(m_seed, offsets, m_min_segment_size,
                                         m_max_segment_size);
#endif
...
struct gen_uniform_key_segments_t {
  ...
  operator()(std::size_t total_elements, std::size_t min_segment_size,
...
struct gen_t {
  vector_generator_t<void>
  operator()(std::size_t elements,
             bit_entropy entropy = bit_entropy::_1_000) const {
    return {seed_t{}, elements, entropy};
  }

  template <class T>
  vector_generator_t<T>
  operator()(std::size_t elements, bit_entropy entropy = bit_entropy::_1_000,
...
template <class T> void do_not_optimize(const T &val) {
  detail::do_not_optimize(&val);
}

struct less_t {
  template <typename DataType>
  __host__ __device__ bool operator()(const DataType &lhs,
...
template <>
__host__ __device__ inline bool less_t::operator()(const complex &lhs,
  ...
  if (cuda::std::isnan(magnitude_0) || cuda::std::isnan(magnitude_1)) {
    ...
    // If the real or imaginary part of the complex number has a very large
    // value (close to the maximum representable value for a double), it is
    // possible that the magnitude computation can result in positive infinity:
    // ```cpp
    // const double large_number = std::numeric_limits<double>::max() / 2;
    // std::complex<double> z(large_number, large_number);
    // std::abs(z) == inf;
    // ```
    // Dividing both components by a constant before computing the magnitude
    // prevents overflow.
  ...
  const complex::value_type difference =
      cuda::std::abs(magnitude_0 - magnitude_1);
  const complex::value_type threshold =
      cuda::std::numeric_limits<complex::value_type>::epsilon() * 2;
...
struct max_t {
  template <typename DataType>
  __host__ __device__ DataType operator()(const DataType &lhs,
                                          const DataType &rhs) {
    less_t less{};
...
#if THRUST_DEVICE_SYSTEM == THRUST_DEVICE_SYSTEM_CUDA
auto policy(caching_allocator_t &alloc) { return thrust::cuda::par(alloc); }
#else
auto policy(caching_allocator_t &) { return thrust::device; }
#endif
...
#else
auto policy(caching_allocator_t &, nvbench::launch &) { return thrust::device; }
#endif
